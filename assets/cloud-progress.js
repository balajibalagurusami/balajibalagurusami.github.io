(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .bits-cloud strong,.bits-cloud small{display:block;color:#fff}.bits-cloud small{margin-top:3px;opacity:.82}
      ...
      .bits-cloud .ok{border-color:#2f9e44}.bits-cloud .google-host{min-height:40px}
  ...
  function renderLocalOnly() {
    ...
    panel.innerHTML = '<div><strong>Progress saved locally</strong><small>Cloud sync is available on wilp.aecbim.work.</small></div>';
  }

  function renderChecking() {
    const panel = ensurePanel();
    panel.innerHTML = '<div><strong>Cloud progress</strong><small>Checking BITS sign-in…</small></div>';
  }

  function renderSigned(user, detail = 'Synced across devices') {
    const panel = ensurePanel();
    panel.innerHTML = `<div><strong>☁️ BITS progress sync on</strong><small>${escapeHtml(user?.email || '')} · ${escapeHtml(detail)}</small></div><button id="bitsLogout" class="ok">Sign out</button>`;
    panel.querySelector('#bitsLogout')?.addEventListener('click', logout);
  }

  function renderUnsigned(message = 'Sign in with your @wilp.bits-pilani.ac.in account to sync across devices.') {
    const panel = ensurePanel();
    panel.innerHTML = `<div><strong>Progress saved locally</strong><small>${escapeHtml(message)}</small></div><div id="bitsGoogleButton" class="google-host"></div>`;
    loadGoogleButton();
  }
  ...
  async function handleGoogleCredential(response) {
    const panel = ensurePanel();
    panel.innerHTML = '<div><strong>Signing in…</strong><small>Verifying BITS Google account.</small></div>';
    try {
      ...
      authenticated = false;
      const msg = error.message === 'wrong_google_workspace' || error.message === 'wrong_email_domain'
        ? 'Only @wilp.bits-pilani.ac.in Google accounts are accepted.'
  ...
  async function init() {
    if (!ACTIVE) return renderLocalOnly();
    renderChecking();
    try {
      const me = await api('/api/me');
      authenticated = !!me.authenticated;
      window.BITS_CLOUD_USER = me.user || {};
      if (!authenticated) return renderUnsigned();
      renderSigned(window.BITS_CLOUD_USER, 'merging this device');
      await syncNow();
    } catch (error) {
