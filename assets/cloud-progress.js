(() => {
  ...
  const API =
    "https://wilptest.aecbim.work";

  const CLIENT_ID =
    "810186188567-7a1mrhng391pmdjjtuemjefekoi9cg48.apps.googleusercontent.com";

  const LIVE_ORIGIN =
    "https://wilp.aecbim.work";
  ...
  function mergeDaily(
    ...
    return {
      ...
      longest:
        Math.max(
          Number(remote.longest) || 0,
          Number(local.longest) || 0
        )
    };
  }
  ...
  function renderLocalOnly() {
    ...
    panel.innerHTML =
      "<div><strong>Progress saved locally</strong><small>Cloud sync is available on wilp.aecbim.work.</small></div>";
  }
  ...
  function renderChecking() {
    ...
    panel.innerHTML =
      "<div><strong>Cloud progress</strong><small>Checking BITS sign-in…</small></div>";
  }

  function renderSigned(
    user,
    detail = "Synced across devices"
  ) {
    const panel =
      ensurePanel();
    ...
    panel.innerHTML = `
      <div>
        <strong>☁️ BITS progress sync on</strong>
  ...
  function renderUnsigned(
    message =
      "Sign in with your @wilp.bits-pilani.ac.in account to sync across devices."
  ) {
    const panel =
      ensurePanel();
  ...
  async function syncNow(
    ...
    try {
      ...
      renderSigned(
        window.BITS_CLOUD_USER,
        `synced · revision ${saved.revision || 0}`
      );
  ...
  async function handleGoogleCredential(
    ...
    panel.innerHTML =
      "<div><strong>Signing in…</strong><small>Verifying BITS Google account.</small></div>";
  ...
  async function init() {
    ...
    try {
      ...
      if (
        ...
        renderUnsigned(
          "Cloud sync is temporarily unavailable; local progress is safe."
        );
      }
    }
  }
